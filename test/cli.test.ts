...
describe("CLI E2E", () => {
  test("aiqt scan on dirty fixtures produces output with findings", async () => {
  ...
  test("aiqt scan with --format json produces valid JSON", async () => {
    const { stdout, exitCode } = await runCli([
      "scan",
      DIRTY_FIXTURES,
      "--format",
  ...
  test("aiqt check on a single dirty file produces findings", async () => {
  ...
  test("aiqt check on a single clean file produces no findings", async () => {
  ...
  test("aiqt scan with --verbose shows timing info", async () => {
  ...
  test("aiqt --version outputs version", async () => {
