...
describe("loadConfig", () => {
  test("loads valid .aiqt.yml", () => {
    const config = loadConfig(join(FIXTURES_DIR, "valid.aiqt.yml"));
  ...
  test("returns defaults when no config file exists", () => {
    const config = loadConfig(join(FIXTURES_DIR, "nonexistent.aiqt.yml"));
  ...
  test("returns defaults for empty YAML file", () => {
    const config = loadConfig(join(FIXTURES_DIR, "empty.aiqt.yml"));
  ...
  test("throws on invalid YAML syntax", () => {
    expect(() =>
      loadConfig(join(FIXTURES_DIR, "bad-yaml.aiqt.yml")),
    ).toThrow(/Invalid YAML/);
  });

  test("throws on invalid config structure", () => {
    expect(() =>
      loadConfig(join(FIXTURES_DIR, "invalid.aiqt.yml")),
    ).toThrow(/Invalid config/);
  });

  test("merges partial config with defaults", () => {
    const config = loadConfig(join(FIXTURES_DIR, "partial.aiqt.yml"));
  ...
  test("handles permission denied gracefully", () => {
    ...
    const config = loadConfig("/nonexistent/path/.aiqt.yml");
